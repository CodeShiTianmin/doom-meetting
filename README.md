# 漫映网络科技 · 多房并发投屏会议系统 (doom-meetting)
...
- **会议时间**：房间显示已进行时长与剩余倒计时；PC 端(管理后台与投屏端房间总览)可按房间设置会议时长，默认 50 分钟(`APP_ROOM_DEFAULT_DURATION_MINUTES`)，剩余 5 分钟/1 分钟推送提醒，到期自动关闭房间。
- **房间聊天**：手机端可发送文字与照片(相册选图，服务端保存在 `APP_CHAT_IMAGE_DIR`，房间关闭时自动删除)，实时广播到房间内各端。
- **点赞**：手机端点赞按钮实时推送 PC 端展示并入库记录。
- **截屏/防录制**：允许截屏、禁止录制——不使用 FLAG_SECURE，采用录屏检测 + 遮挡上报方案。
- **缺人红灯预警**：创建房间后缺人状态超过 3 分钟（可配置），后台/PC 端房间卡亮红灯预警（不弹窗）；心跳超时自动判定离线，成员退出或离线后人名立即置灰。
- **计时复位**：PC 端房间总览可单独将运行中房间的会议计时复位到 00:00（不结束会议、不影响成员与推流）。
