...
- **就位与播放控制**：全部手机客户端就位后房间进入"已运行"并开始会议计时；手机端可 开始播放/暂停/拖拉进度条（指令带序号、后端串行转发防两端冲突），明暗/音量为本地调节。
...
全链路 HTTPS/WSS；媒体流 WebRTC DTLS-SRTP 加密；SFU 关闭录制/转推，服务器不存储任何媒体内容（零留存）；一次性入会凭证（短时效 + 绑定房间 + 限用次数）；房间人数硬限制（成员数上限可按房间设置，默认 2）。
