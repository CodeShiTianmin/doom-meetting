...
class FlutterScreenCapture : public MediaListObserver,
 ...
 private:
  ...
  // Loopback audio sessions keyed by the desktop capturer they belong to.
  // Multiple screen-share sessions can each capture their own (per-process)
  // system audio concurrently. Empty on platforms without loopback support.
  struct LoopbackSession {
    std::unique_ptr<LoopbackCapturer> capturer;
    scoped_refptr<RTCAudioSource> source;
  };
  std::map<RTCDesktopCapturer*, LoopbackSession> loopback_sessions_;
};
