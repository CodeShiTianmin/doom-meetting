...
void FlutterScreenCapture::OnStop(scoped_refptr<RTCDesktopCapturer> capturer) {
  auto it = loopback_sessions_.find(capturer.get());
  if (it != loopback_sessions_.end()) {
    it->second.capturer->Stop();
    loopback_sessions_.erase(it);
  }
}
...
void FlutterScreenCapture::GetDisplayMedia(
  ...
  if (capture_audio) {
    ...
    scoped_refptr<RTCAudioSource> loopback_audio_source =
    ...
    std::unique_ptr<LoopbackCapturer> loopback_capturer =
